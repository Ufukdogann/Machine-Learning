...
class Linear:
    ...
    def backward(self, error):
        ...
        dW = np.matmul(x_transpose, error)
        vector_of_one = np.ones((error.shape[0]))
        db = np.matmul(vector_of_one, error)
        d_inputs = np.matmul(error, w_transpose)
...
class Sequential:
    ...
    def backward(self, error):
        ...
        for module_index in reversed(range(len(self.modules))):
            ...
            module_variable_grad = error[0]
            module_input_grad = error[1]
...
class MSE:
    ...
    def backward(self):
        ...
        T = self.saved_variables["target"]
        n = self.saved_variables["prediction.size"]
...
def train_one_step(model, loss, learning_rate, inputs, targets):
    ...
    error = loss.forward(forward_pass, targets)

    variable_gradients, _ = model.backward(loss.backward())

    for module_index in reversed(range(len(model.modules))):
        if (module_index % 2) == 0:
            module = model.modules[module_index]

            new_weight = module.var["W"] - (learning_rate * variable_gradients["mod_%d.W" % module_index])
            module.var["W"] = new_weight

            new_bias = module.var["b"] - (learning_rate * variable_gradients["mod_%d.b" % module_index])
            module.var["b"] = new_bias
