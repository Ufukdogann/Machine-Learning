...
split_number = n_train
train_indices = indices[:split_number]
validation_indices = indices[split_number:]
...
def validate():
    ...
    for i in range(0, len(validation_indices), batch_size):
        ...
        num_ok, total_num = verify(output, labels)
        accu = accu + num_ok
...
for epoch in range(1000):
    ...
    error = 0
    batch_count = len(train_indices) / batch_size
    ...
    if(validation_accuracy > best_validation_accuracy):
        ...
        best_epoch = epoch
    elif(epoch == best_epoch + 10 and best_validation_accuracy > validation_accuracy ):
